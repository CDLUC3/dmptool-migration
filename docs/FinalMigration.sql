...
INSERT INTO dmptool.users (givenName, surName, affiliationId, password,
                           ...
                           notify_on_template_shared, notify_on_feedback_complete, notify_on_plan_shared,
                           notify_on_plan_visibility_change, created, modified)
VALUES ('Test', 'Researchers', 'https://ror.org/03yrm5c26',
        '$2a$10$f3wCBdUVt/2aMcPOb.GX1OBO9WMGxDXx5HKeSBBnrMhat4.pis4Pe',
...
INSERT dmptool.users (givenName, surName, affiliationId, password,
                           ...
                           notify_on_template_shared, notify_on_feedback_complete, notify_on_plan_shared,
                           notify_on_plan_visibility_change, created, modified)
VALUES ('NSF', 'Admin', 'https://ror.org/021nxhr62',
        '$2a$10$f3wCBdUVt/2aMcPOb.GX1OBO9WMGxDXx5HKeSBBnrMhat4.pis4Pe',
        'ADMIN', 1, 0, 1, 'en-US', 1, 1, 1, 1, 1, CURRENT_DATE(), CURRENT_DATE());


SELECT COUNT(*) FROM dmptool.users; -- 141,269 as of 2025-11-21
...
INSERT INTO dmptool.userEmails (userId, email, isPrimary, isConfirmed, created, createdById,
                                modified, modifiedById)
    (SELECT id, 'researcher@example.gov', 1, 1, CURRENT_DATE(), id, CURRENT_DATE, id
...
INSERT INTO dmptool.userEmails (userId, email, isPrimary, isConfirmed, created, createdById,
                                modified, modifiedById)
    (SELECT id, 'admin@nsf.gov', 1, 1, CURRENT_DATE(), id, CURRENT_DATE, id
     FROM dmptool.users WHERE givenName = 'NSF' AND surName = 'Admin');

SELECT COUNT(*) FROM dmptool.userEmails; -- 141,269 as of 2025-11-21
...
SELECT COUNT(*) FROM dmptool.affiliations; -- 122,148 as of 2025-11-21
...
WHERE affiliationId IN (SELECT uri FROM migration.affiliations);  -- 937 as of 2025-11-21
...
WHERE affiliationId IN (SELECT uri FROM migration.affiliations);  -- 5,077 as of 2025-11-21
...
WHERE affiliationId IN (SELECT uri FROM migration.affiliations);   -- 6,357 as of 2025-11-21
...
FROM migration.research_domains;   -- 48 row as of 2025-11-21
...
FROM migration.tags;   -- 14 rows as of 2025-11-21
...
SELECT id, label, uri, description, displayOrder, isDefault,
       createdById, created, modifiedById, modified
FROM migration.member_roles;   -- 15 rows as of 2025-11-21
...
FROM migration.repositories;   -- 4,117 rows as of 2025-11-21
...
FROM migration.metadata_standards;   -- 289 rows as of 2025-11-21


-- Migrate the licenses
INSERT INTO dmptool.licenses (name, uri, description, createdById, created, modifiedById, modified)
SELECT DISTINCT name, uri, description, createdById, created, modifiedById, modified
FROM migration.licenses;   -- 680 rows as of 2025-11-21

-- Set the default recommended licenses
update licenses set recommended = 1 where id in (71, 101, 229);
...
FROM migration.templates;    -- 471 rows as of 2025-11-21
...
FROM migration.template_links;    -- 211 rows as of 2025-11-21
...
FROM migration.versioned_templates;    -- 1,126 rows as of 2025-11-21
...
FROM migration.versioned_template_links;    -- 601 rows as of 2025-11-21
...
SELECT id, templateId, name, introduction, displayOrder, bestPractice, isDirty, created, createdById,
       modified, modifiedById
FROM migration.sections;    -- 2,336 rows as of 2025-11-21
...
FROM migration.section_tags;    -- 5,090 rows as of 2025-11-21
...
SELECT id, versionedTemplateId, sectionId, name, introduction, displayOrder, bestPractice, created, createdById,
       modified, modifiedById
FROM migration.versioned_sections;    -- 8,033 rows as of 2025-11-21
...
FROM migration.versioned_section_tags;    -- 17,506 rows as of 2025-11-21
...
SELECT id, templateId, sectionId, displayOrder, isDirty, questionText, json, guidanceText, sampleText,
       created, createdById, modified, modifiedById
FROM migration.questions;    -- 5,583 rows as of 2025-11-21
...
FROM migration.versioned_questions;    -- 19,271 rows as of 2025-11-21

-- Migrate the guidance
INSERT IGNORE INTO dmptool.guidanceGroups (id, affiliationId, name, isDirty, optionalSubset, bestPractice, latestPublishedVersion, latestPublishedDate,
                                    created, createdById, modified, modifiedById)
SELECT id, affiliationId, name, isDirty, optionalSubset, bestPractice, latestPublishedVersion, latestPublishedDate,
       created, createdById, modified, modifiedById
FROM migration.guidance_groups; -- 2,418 rows as of 2025-11-21

INSERT IGNORE INTO dmptool.guidance (id, guidanceGroupId, guidanceText, created, createdById, modified, modifiedById)
SELECT id, guidanceGroupId, guidanceText, created, createdById, modified, modifiedById
FROM migration.guidance; -- 1,661 rows as of 2025-11-21

INSERT IGNORE INTO dmptool.guidanceTags (id, guidanceId, tagId, created, createdById, modified, modifiedById)
SELECT id, guidanceId, tagId, created, createdById, modified, modifiedById
FROM migration.guidance_tags; -- 2,280 rows as of 2025-11-21

INSERT IGNORE INTO dmptool.versionedGuidanceGroups (id, guidanceGroupId, name, active, optionalSubset, bestPractice,
													version, created, createdById, modified, modifiedById)
SELECT id, guidanceGroupId, name, active, optionalSubset, bestPractice, version,
       created, createdById, modified, modifiedById
FROM migration.versioned_guidance_groups; -- 273 rows as of 2025-11-21

INSERT IGNORE INTO dmptool.versionedGuidance (id, versionedGuidanceGroupId, guidanceId, guidanceText, created, createdById, modified, modifiedById)
SELECT id, versionedGuidanceGroupId, guidanceId, guidanceText, created, createdById, modified, modifiedById
FROM migration.versioned_guidance; -- 1,502 rows as of 2025-11-21

INSERT IGNORE INTO dmptool.versionedGuidanceTags (id, versionedGuidanceId, tagId, created, createdById, modified, modifiedById)
SELECT id, versionedGuidanceId, tagId, created, createdById, modified, modifiedById
FROM migration.versioned_guidance_tags; -- 2,059 rows as of 2025-11-21
...
SELECT id, title, abstractText, researchDomainId, startDate, endDate, isTestProject, created, createdById,
       modified, modifiedById
FROM migration.projects;   -- 136,237 rows as of 2025-11-21
...
FROM migration.project_collaborators;   -- 147,401 rows as of 2025-11-21
...
SELECT DISTINCT id, projectId, affiliationId, givenName, surName, email, orcid,
                isPrimaryContact, created, createdById, modified, modifiedById
FROM migration.final_project_members AS pm;   -- 171,205 as of 2025-11-21
...
FROM migration.project_member_roles;   -- 511,622 rows as of 2025-11-21
...
SELECT id, title, abstractText, startDate, endDate, isTestProject, created, createdById,
       modified, modifiedById
FROM migration.draft_projects;  -- 77 rows as of 2025-11-24

-- Migrate pilot project collaborators
INSERT IGNORE INTO dmptool.projectCollaborators (projectId, email, invitedById, userId, accessLevel,
                                          created, createdById, modified, modifiedById)
SELECT DISTINCT projectId, email, invitedById, userId, accessLevel, created, createdById, modified, modifiedById
FROM migration.draft_project_collaborators;  -- 77 rows as of 2025-11-24
...
SELECT projectId, affiliationId, status, funderProjectNumber, grantId, funderOpportunityNumber,
       created, createdById, modified, modifiedById
FROM migration.draft_project_fundings;  -- 68 rows as of 2025-11-24

-- Migrate pilot project members
INSERT INTO dmptool.projectMembers (id, projectId, affiliationId, givenName, surName, email, orcid,
										   isPrimaryContact, created, createdById, modified, modifiedById)
SELECT DISTINCT id, projectId, affiliationId, givenName, surName, email, orcid,
                isPrimaryContact, created, createdById, modified, modifiedById
FROM migration.draft_project_members AS pm; -- 50 rows as of 2025-11-24

-- Migrate pilot project member roles
INSERT IGNORE INTO dmptool.projectMemberRoles (projectMemberId, memberRoleId, created, createdById, modified, modifiedById)
SELECT DISTINCT projectMemberId, memberRoleId, created, createdById, modified, modifiedById
FROM migration.draft_project_member_roles;  -- 92 rows as of 2025-11-24

-- Ensure each draft project has a primary contact
WITH projects_no_primary_contact AS (
    SELECT pm.id projectId, MAX(pm.isPrimaryContact) AS max_primary
    FROM migration.draft_projects dp JOIN dmptool.projectMembers pm ON dp.id = pm.projectId
    GROUP BY dp.id, dp.title, pm.id, pm.email
    HAVING max_primary = 0
    ORDER BY pm.id
)
UPDATE dmptool.projectMembers AS pm JOIN migration.draft_projects AS dp ON pm.projectId = dp.id
    SET pm.isPrimaryContact = 1
WHERE pm.id IN (SELECT pnpc.projectId FROM projects_no_primary_contact AS pnpc);

-- TODO: Determine why some versionedTemplateId are NULL. Related to missing affiliations?

-- Migrate the plans
INSERT IGNORE INTO dmptool.plans (id, projectId, versionedTemplateId, title, visibility, status, dmpId, registeredById,
						   registered, languageId, featured, created, createdById, modified, modifiedById)
SELECT id, projectId, versionedTemplateId, title, visibility, status, dmpId, registeredById, registered,
       languageId, featured, created, createdById, modified, modifiedById
FROM migration.plans;    -- 133,018 rows as of 2025-11-21
...
SELECT planId, versionedSectionId, versionedQuestionId, json, createdById, created, modifiedById, modified
FROM migration.answers
WHERE planId IS NOT NULL; -- 586,906 rows as of 2025-11-21
...
FROM migration.plan_members;    -- 167,708 rows as of 2025-11-21
...
FROM migration.plan_member_roles;    -- 501,352 rows as of 2025-11-21
...
SELECT id, id, (SELECT id FROM dmptool.versionedTemplates WHERE bestPractice = 1 AND active = 1 LIMIT 1), title, visibility,
  CASE WHEN dmp_id IS NOT NULL THEN 'COMPLETE' ELSE 'DRAFT' END,
  dmp_id,
  CASE WHEN dmp_id IS NOT NULL THEN modifiedById ELSE NULL END,
  CASE WHEN dmp_id IS NOT NULL THEN modified ELSE NULL END,
  'en-US',
  0, created, createdById, modified, modifiedById
FROM migration.draft_projects;    -- 77 rows as of 2025-11-24

-- Migrate pilot project plan funding
INSERT IGNORE INTO dmptool.planFundings (planId, projectFundingId, created, createdById, modified, modifiedById)
SELECT dpf.projectId, pf.id, dpf.created, dpf.createdById, dpf.modified, dpf.modifiedById
FROM migration.draft_project_fundings AS dpf
  JOIN dmptool.projectFundings AS pf ON dpf.projectId = pf.projectId;  -- 68 rows as of 2025-11-24

-- Migrate pilot project plan members
INSERT IGNORE INTO dmptool.planMembers (id, planId, projectMemberId, isPrimaryContact, created, createdById, modified,
								 modifiedById)
SELECT pm.id, pm.projectId, pm.id, pm.isPrimaryContact, pm.created, pm.createdById, pm.modified, pm.modifiedById
FROM dmptool.projectMembers AS pm
  JOIN migration.draft_projects AS dp ON pm.projectId = dp.id; -- 50 rows as of 2025-11-24


-- Migrate pilot project plan member roles
INSERT IGNORE INTO dmptool.planMemberRoles (planMemberId, memberRoleId, created, createdById, modified, modifiedById)
SELECT pmr.id, pmr.memberRoleId, pmr.created, pmr.createdById, pmr.modified, pmr.modifiedById
FROM dmptool.projectMemberRoles AS pmr
  JOIN dmptool.projectMembers AS pm ON pmr.projectMemberId = pm.id
    JOIN migration.draft_projects AS dp ON pm.projectId = dp.id;  -- 92 rows as of 2025-11-24
...
GROUP BY rw.value;   -- 321 rows as of 2025-11-21
...
WHERE migration.related_works.identifier_type = 'DOI' AND migration.related_works.is_valid = 1;   -- 321 rows as of 2025-11-21
...
FROM migration.related_works AS rw
         ...
         INNER JOIN migration.plans As p ON rw.plan_id = p.old_plan_id; -- 328 rows as of 2025-11-21
