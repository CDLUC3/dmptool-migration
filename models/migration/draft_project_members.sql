...
SELECT DISTINCT
  ...
  CASE WHEN pm.email IS NOT NULL AND pm.orcid IS NULL THEN
    (SELECT fpm.orcid FROM migration.final_project_members fpm WHERE pm.email = fpm.email ORDER BY fpm.orcid DESC LIMIT 1)
    ELSE pm.orcid
  END AS orcid,
  pm.email,
