...
MODEL (
  ...
  audits (
    not_null(columns := (old_template_id, new_template_id, linkType, url, text))
  ),
  enabled true
);
...
SELECT
  ...
  tmplt.new_template_id AS new_template_id,
  'FUNDER' AS linkType,
...
FROM dmp.templates t
  JOIN intermediate.templates tmplt ON t.id = tmplt.old_template_id
  JOIN JSON_TABLE(
    t.links,
    '$.funder[*]'
    COLUMNS(
